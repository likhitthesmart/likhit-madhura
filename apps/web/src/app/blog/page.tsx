      ...
      <div className="mt-10 flex flex-wrap justify-center gap-2">
        {categories.map((c) => (
          <Link
            ...
            className={cn("rounded-full border px-5 py-2 text-sm font-medium transition", category === c.v ? "border-forest-700 bg-deep-800 text-ivory" : "border-sand-dark text-bark hover:border-forest-400")}
          >
            {c.l}
          </Link>
        ))}
      ...
      <Stagger className="mt-14 grid gap-8 md:grid-cols-2 lg:grid-cols-3">
        {posts.map((p) => (
          <StaggerItem key={p.slug}>
            <Link href={`/blog/${p.slug}`} className="group block overflow-hidden rounded-organic border border-sand bg-surface shadow-card transition-all duration-500 hover:-translate-y-1 hover:shadow-lift">
